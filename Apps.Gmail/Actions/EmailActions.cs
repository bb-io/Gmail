...
public class EmailActions(InvocationContext invocationContext, IFileManagementClient fileManagementClient) : GmailInvocable(invocationContext)
{
    ...
    {
        var email = await ExecuteWithErrorHandlingAsync(Client.Users.Messages.Get("me", getEmailRequest.EmailId).ExecuteAsync);
        ...
        if (email.Payload.Parts != null)
        {
            ...
            {
                var att = await ExecuteWithErrorHandlingAsync(Client.Users.Messages.Attachments.Get("me", getEmailRequest.EmailId, part.Body.AttachmentId).ExecuteAsync);
    ...
    {
        await ExecuteWithErrorHandlingAsync(Client.Users.Messages.Trash("me", getEmailRequest.EmailId).ExecuteAsync);           
    }
    ...
    public async Task<EmailDto> SendEmail([ActionParameter] SendEmailRequest sendEmailRequest)
    {
        ...
        var email = await ExecuteWithErrorHandlingAsync(Client.Users.Messages.Send(message, "me").ExecuteAsync);
